...
def test_remaining_work_graph_check_blocks_ready_slice_missing_initiative_metadata(tmp_path: Path) -> None:
    ...
    assert code == 1
    assert "slice_missing_integration_mode:rwg-006" in report["errors"]
    assert "ready_slice_wrong_integration_mode:rwg-006:" in report["errors"]
